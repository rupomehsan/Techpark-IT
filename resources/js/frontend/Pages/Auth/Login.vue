  ...
  <div class="login-container">
    <div class="login-card">
      ...
      <div class="login-header">
        ...
        <h1 class="login-title">My Dictionary</h1>
        ...
        <div class="form-wrapper">
          ...
          <form @submit.prevent="LoginSubmitHandler" class="login-form">
            ...
            <div class="form-group">
              ...
              <input v-model="email" type="email" name="email" class="form-input" placeholder="test@gmail.com" required />
            </div>
            ...
            <button type="submit" class="signin-btn">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M10,17L15,12L10,7V10H4V14H10V17Z" />
              </svg>
              Sign In
            </button>
          </form>
...
export default {
  ...
  created: async function () {
    ...
    if (token && role) {
      try {
        ...
        if (this.is_auth) {
          ...
          console.log("User already authenticated, redirecting...", { role: userRole, prevUrl });

          if (userRole === 1 || role === "1") {
            const redirectUrl = prevUrl && prevUrl.startsWith("/super-admin") ? "/super-admin" + prevUrl : "/super-admin#/dashboard";
            window.location.href = redirectUrl;
            return;
          } else if (userRole === 3 || role === "3") {
            const redirectUrl = prevUrl && prevUrl.startsWith("/profile") ? "/profile" + prevUrl : "/profile";
            window.location.href = redirectUrl;
            return;
          }
        } else {
    ...
    // If no token, invalid auth, or auth check failed, user stays on login page
        ...
        if (response.data?.status === "success") {
          ...
          if (data.access_token && data.user) {
            ...
            if (data.user.role_id === 1) {
              redirectUrl = prevUrl && prevUrl.startsWith("/super-admin") ? prevUrl : "/super-admin#/dashboard";
            } else if (data.user.role_id === 3) {
              redirectUrl = prevUrl && prevUrl.startsWith("/profile") ? prevUrl : "/profile";
            }
