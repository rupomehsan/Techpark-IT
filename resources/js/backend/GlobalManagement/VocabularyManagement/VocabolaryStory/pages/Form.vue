    ...
    <form @submit.prevent="submitHandler">
      <div class="card">
        ...
        <div class="card-body card_body_fixed_height">
          <div class="row">
            <vocabulary-drop-down-el :key="`words-` + resetKey" ref="wordsDropdown" :name="'words'" :multiple="true" :value="item.words" />
            <template v-for="(form_field, index) in form_fields" v-bind:key="index">
              <common-input
                ...
                :item="this.item"
                :api_url="form_field.api_url"
              />
            </template>
            <multiple-image-uploader ref="storyImageUploader" :name="'story_image'" :multiple="true" :images="item.story_image" />
            <multi-chip ref="multiChip" :key="`keyword-` + resetKey" :name="`keyword`" :value="item.keyword" />
          </div>
        </div>
...
import VocabularyDropDownEl from "../../Vocabulary/components/dropdown/DropDownEl.vue";
...
export default {
  ...
  created: async function () {
    ...
    if (id) {
      ...
      if (typeof this.clear_item === 'function') this.clear_item();
    }
  },
  methods: {
    ...mapActions(store, {
    ...
    submitHandler: async function ($event) {
      ...
      if (this.param_id) {
        ...
        if ([200, 201].includes(response.status)) {
          ...
          // Reset custom child components: dropdown, multi-chip, image uploader
          this.resetKey = this.resetKey + 1;
          // Reset vocabulary dropdown if it exposes a reset method
          if (this.$refs.wordsDropdown && typeof this.$refs.wordsDropdown.reset === 'function') {
            this.$refs.wordsDropdown.reset();
          }
          // Ensure parent model is cleared so the child receives empty value as prop
          if (this.item) {
            this.item.words = [];
            // also clear keyword and story_image so multi-chip and uploader clear
            this.item.keyword = Array.isArray(this.item.keyword) ? [] : '';
            this.item.story_image = [];
          }
          if (this.$refs.multiChip && typeof this.$refs.multiChip.reset === 'function') {
            this.$refs.multiChip.reset();
          }
          if (this.$refs.storyImageUploader && typeof this.$refs.storyImageUploader.reset === 'function') {
            this.$refs.storyImageUploader.reset();
          }
        }
      }
    },
