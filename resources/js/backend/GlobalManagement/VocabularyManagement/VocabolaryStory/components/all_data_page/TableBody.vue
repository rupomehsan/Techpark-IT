  ...
  <tr v-for="(item, dataindex) in data" :key="item.id" :class="`table_rows table_row_${item.id}`">
    ...
    <template v-for="(row_item, index) in setup.table_row_data" :key="index">
      ...
      <td v-else class="text-wrap max-w-120">
        <div v-if="isHtmlString(item[row_item])" v-html="item[row_item]"></div>
...
export default {
  ...
  methods: {
    ...
    isHtmlString(content) {
      if (!content || typeof content !== 'string') return false;
      // Quick heuristic: contains HTML tags like <p>, <br>, <div>, <span>, <strong>, etc.
      // Avoid false positives by ensuring there's a matching closing bracket.
      const htmlTagPattern = /<([a-zA-Z]+)([^>]*?)>(.*?)<\/\1>/s;
