    ...
    <template v-for="(row_item, index) in setup.table_row_data" :key="index">
      ...
      <td v-else-if="isFileField(item[row_item])" class="text-wrap max-w-120">
        <template v-if="isImageFile(item[row_item])">
          <a :href="item[row_item] || '/avatar.png'" data-fancybox="gallery" :data-caption="`${row_item} - Image ${dataindex + 1}`">
            <img
              :src="item[row_item] || '/avatar.png'"
              @error="handleImageError($event)"
        ...
        <template v-else>
          <a :href="item[row_item]" target="_blank" class="file-download-link" :title="`Download ${getFileName(item[row_item])}`">
            <i class="fas fa-file-download"></i>
            {{ getFileName(item[row_item]) }}
          </a>
        </template>
      </td>
      <td v-else class="text-wrap max-w-120" :title="trim_content(item[row_item], row_item)">
