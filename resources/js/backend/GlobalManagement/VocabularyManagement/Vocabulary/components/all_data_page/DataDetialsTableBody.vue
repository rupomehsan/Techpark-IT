  ...
  <template v-for="(row_item, index) in filteredFields" :key="index">
    <tr>
      ...
      <th class="text-trim">
        ...
        <template v-else>
          <div v-if="row_item === 'example_sentence'">
            <div v-if="Array.isArray(item[row_item]) && item[row_item].length">
              <div v-for="(ex, idx) in item[row_item]" :key="idx" class="mb-2">
                <div><strong>Meaning (BN):</strong> {{ ex.meaning }}</div>
                <div><strong>Sentence (EN):</strong> {{ ex.sentence }}</div>
              </div>
            </div>
            <div v-else>
