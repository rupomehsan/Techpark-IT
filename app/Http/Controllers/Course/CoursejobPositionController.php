...
class CoursejobPositionController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $data = CourseJobPositions::where('id', $id)
            ->select($select)
            ->first();
        if ($data) {
    ...
    {
        $data = CourseJobPositions::find(request()->id);
        if(!$data){
            return response()->json([
                'err_message' => 'validation error',
    ...
    {
        $validator = Validator::make(request()->all(), [
            'id' => ['required','exists:course_job_positions,id'],
        ]);
    ...
    public function bulk_import()
    {
        ...
        foreach (request()->data as $item) {
            ...
            $item = (object) $item;
            $check = CourseJobPositions::where('id',$item->id)->first();
            if(!$check){
                try {
                    CourseJobPositions::create((array) $item);
                } catch (\Throwable $th) {
                    return response()->json([
                        'err_message' => 'validation error',
