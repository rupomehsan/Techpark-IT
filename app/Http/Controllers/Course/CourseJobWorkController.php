...
class CourseJobWorkController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $data = CourseJobWorks::where('id', $id)
            ->select($select)
            ->first();
        if ($data) {
    ...
    {
        $data = CourseJobWorks::find(request()->id);
        if(!$data){
            return response()->json([
                'err_message' => 'validation error',
    ...
    {
        $validator = Validator::make(request()->all(), [
            'id' => ['required','exists:course_job_works,id'],
        ]);
    ...
    public function bulk_import()
    {
        ...
        foreach (request()->data as $item) {
            ...
            $item = (object) $item;
            $check = CourseJobWorks::where('id',$item->id)->first();
            if(!$check){
                try {
                    CourseJobWorks::create((array) $item);
                } catch (\Throwable $th) {
                    return response()->json([
                        'err_message' => 'validation error',
