...
class CourseFaqsController extends Controller
{
    public function all($course_id)
    {
        ...
        $query = CourseFaqs::where('status', $status)->where('course_id', $course_id)->orderBy($orderBy, $orderByType);
    ...
    {
        $data = CourseFaqs::find(request()->id);
        if(!$data){
            return response()->json([
                'err_message' => 'validation error',
    ...
    {
        $validator = Validator::make(request()->all(), [
            'id' => ['required','exists:course_faqs,id'],
        ]);
    ...
    public function bulk_import()
    {
        ...
        foreach (request()->data as $item) {
            ...
            $item = (object) $item;
            $check = CourseFaqs::where('id',$item->id)->first();
            if(!$check){
                try {
                    CourseFaqs::create((array) $item);
                } catch (\Throwable $th) {
                    return response()->json([
                        'err_message' => 'validation error',
